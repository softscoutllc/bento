/**
 * SoftScout PDF CORS Proxy Worker
 *
...
const ALLOWED_ORIGINS = ['https://softscout.net', 'https://softscout.net'];
  ...
  async fetch(request, env, ctx) {
    ...
    if (!isAllowedOrigin(origin)) {
      return new Response(
        JSON.stringify({
          error: 'Forbidden',
          message: 'This proxy only accepts requests from softscout.net',
        }),
        {
          status: 403,
          headers: {
    ...
    try {
      const response = await fetch(targetUrl, {
        headers: {
          'User-Agent': 'SoftScout PDF-CertProxy/1.0',
        },
      });
